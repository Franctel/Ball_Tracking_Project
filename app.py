...
def extract_frames(video_path):
    ...
    while cap.isOpened():
        ...
        # Resize to reduce memory (but no frame limit)
        frame = cv2.resize(frame, (1280, 720))  # Resize to HD

        # Apply ROI logic as before
        roi = frame[:min(800, frame.shape[0]), :]
        if roi.shape[1] > 2400:
            roi = roi[:, 1200:roi.shape[1]-1200]

        # Try-catch to avoid crash if frame is corrupt
        try:
            cv2.imwrite(f"{app.config['FRAME_FOLDER']}/{cnt}.png", roi)
        except Exception as e:
            print(f"Frame {cnt} write failed: {e}")
        cnt += 1
