...
roiCanvas.addEventListener('mousemove', (e) => {
  ...
  roi = {x: Math.min(startX, mouseX), y: Math.min(startY, mouseY), width: Math.abs(width), height: Math.abs(height)};
  drawRoiRect();
});
...
setRoiBtn.addEventListener('click', () => {
  ...
  fetch('/set_roi',{
    ...
    if(data.success){
      roiCoordsSpan.textContent = "x=" + roi.x + ", y=" + roi.y + ", w=" + roi.width + ", h=" + roi.height;
